/*
Auto-generated by: https://github.com/pmndrs/gltfjsx
Command: npx gltfjsx@6.5.3 ./public/models/building.glb --types --shadows 
*/

import * as THREE from 'three'
import React, { useEffect, useRef } from 'react'
import { useGraph } from '@react-three/fiber'
import { useGLTF, useAnimations } from '@react-three/drei'
import { GLTF, SkeletonUtils } from 'three-stdlib'

type ActionName =
  | 'Event_About_01_Loop'
  | 'Event_About_01_Start'
  | 'Event_Skills_01_Loop'
  | 'Event_Skills_01_Start'
  | 'Event_Skills_02_Loop'
  | 'Event_Skills_02_Start'
  | 'Event_Skills_03_Loop'
  | 'Event_Skills_03_Start'
  | 'Event_Skills_04_Loop'
  | 'Event_Skills_04_Start'
  | 'Event_Projects_02_Loop'
  | 'Event_Projects_02_Start'
  | 'Event_Projects_01_Loop'
  | 'Event_Projects_01_Start'
  | 'Event_Projects_03_Loop'
  | 'Event_Projects_03_Start'
  | 'Event_Gallery_02_Loop'
  | 'Event_Gallery_02_Start'
  | 'Event_Gallery_01_Loop'
  | 'Event_Gallery_01_Start'
  | 'Event_Gallery_03_Loop'
  | 'Event_Gallery_03_Start'
  | 'Event_Gallery_04_Loop'
  | 'Event_Gallery_04_Start'
  | '丸型エレベータードア.LAction.001'
  | '丸型エレベータードア.RAction.001'
  | '丸型エレベータードア.LAction'
  | '丸型エレベータードア.RAction'
  | '丸型エレベータードア.LAction.002'
  | '丸型エレベータードア.RAction.002'
  | 'wave_react'
  | 'wave_python'
  | 'Wave_blender'
  | 'wave_ts'
  | '丸型エレベータードア.LAction.003'
  | '丸型エレベータードア.RAction.003'
  | '丸型エレベータードア.LAction.004'
  | '丸型エレベータードア.RAction.004'
  | 'Fire'

interface BuildingProps {
  activeEvent: string | null
}

interface GLTFAction extends THREE.AnimationClip {
  name: ActionName
}

type GLTFResult = GLTF & {
  nodes: {
    外枠003: THREE.SkinnedMesh
    外枠001: THREE.SkinnedMesh
    外枠004: THREE.SkinnedMesh
    外枠006: THREE.SkinnedMesh
    外枠002: THREE.SkinnedMesh
    外枠005: THREE.SkinnedMesh
    外枠007: THREE.SkinnedMesh
    外枠008: THREE.SkinnedMesh
    外枠009: THREE.SkinnedMesh
    外枠010: THREE.SkinnedMesh
    外枠011: THREE.SkinnedMesh
    外枠012: THREE.SkinnedMesh
    fire: THREE.SkinnedMesh
    柵: THREE.Mesh
    屋上床: THREE.Mesh
    屋上ライト: THREE.Mesh
    屋上ライト001: THREE.Mesh
    屋上ライト002: THREE.Mesh
    屋上ライト003: THREE.Mesh
    望遠鏡: THREE.Mesh
    丸型エレベータードアL001: THREE.Mesh
    丸型エレベータードアR001: THREE.Mesh
    エレベーター: THREE.Mesh
    円柱030_1: THREE.Mesh
    円柱030_2: THREE.Mesh
    Cube004: THREE.Mesh
    Cube005_1: THREE.Mesh
    Cube005_2: THREE.Mesh
    Cube005_3: THREE.Mesh
    Cube006: THREE.Mesh
    Cube007: THREE.Mesh
    立方体012: THREE.Mesh
    円柱022: THREE.Mesh
    円柱022_1: THREE.Mesh
    円柱022_2: THREE.Mesh
    円柱022_3: THREE.Mesh
    立方体020: THREE.Mesh
    立方体020_1: THREE.Mesh
    立方体020_2: THREE.Mesh
    立方体020_3: THREE.Mesh
    立方体020_4: THREE.Mesh
    立方体020_5: THREE.Mesh
    立方体020_6: THREE.Mesh
    マウス002: THREE.Mesh
    円柱014: THREE.Mesh
    円柱023: THREE.Mesh
    円柱023_1: THREE.Mesh
    円柱023_2: THREE.Mesh
    立方体013: THREE.Mesh
    立方体014: THREE.Mesh
    立方体015: THREE.Mesh
    立方体021: THREE.Mesh
    立方体022: THREE.Mesh
    立方体023: THREE.Mesh
    立方体040_1: THREE.Mesh
    立方体040_2: THREE.Mesh
    本001: THREE.Mesh
    本棚001: THREE.Mesh
    立方体044_1: THREE.Mesh
    立方体044_2: THREE.Mesh
    Floor: THREE.Mesh
    立方体045_1: THREE.Mesh
    立方体045_2: THREE.Mesh
    本004: THREE.Mesh
    本005: THREE.Mesh
    本006: THREE.Mesh
    立方体022_1: THREE.Mesh
    立方体022_2: THREE.Mesh
    円柱025_1: THREE.Mesh
    円柱025_2: THREE.Mesh
    立方体025: THREE.Mesh
    丸型エレベータードアL: THREE.Mesh
    丸型エレベータードアR: THREE.Mesh
    円柱012: THREE.Mesh
    ガラス窓: THREE.Mesh
    建物001: THREE.Mesh
    平面_1: THREE.Mesh
    平面_2: THREE.Mesh
    丸型エレベータードアL002: THREE.Mesh
    丸型エレベータードアR002: THREE.Mesh
    円柱017: THREE.Mesh
    立方体044: THREE.Mesh
    円柱025: THREE.Mesh
    円柱026: THREE.Mesh
    円柱027: THREE.Mesh
    立方体029: THREE.Mesh
    立方体027: THREE.Mesh
    立方体036_1: THREE.Mesh
    立方体036_2: THREE.Mesh
    立方体036_3: THREE.Mesh
    ['python-logo-only_1']: THREE.Mesh
    ['python-logo-only_2']: THREE.Mesh
    立方体037_1: THREE.Mesh
    立方体037_2: THREE.Mesh
    立方体037_3: THREE.Mesh
    立方体037_4: THREE.Mesh
    ['icons8-typescript-96']: THREE.Mesh
    ['icons8-typescript-96_1']: THREE.Mesh
    ['icons8-typescript-96_2']: THREE.Mesh
    ['icons8-typescript-96_3']: THREE.Mesh
    ['icons8-typescript-96_4']: THREE.Mesh
    ['icons8-typescript-96_5']: THREE.Mesh
    立方体100: THREE.Mesh
    立方体100_1: THREE.Mesh
    立方体035: THREE.Mesh
    立方体036: THREE.Mesh
    立方体037: THREE.Mesh
    立方体038: THREE.Mesh
    立方体039: THREE.Mesh
    立方体040: THREE.Mesh
    立方体041: THREE.Mesh
    立方体042: THREE.Mesh
    立方体043: THREE.Mesh
    円柱037: THREE.Mesh
    円柱037_1: THREE.Mesh
    円柱042: THREE.Mesh
    円柱042_1: THREE.Mesh
    円柱043: THREE.Mesh
    円柱043_1: THREE.Mesh
    円柱044: THREE.Mesh
    円柱044_1: THREE.Mesh
    立方体060: THREE.Mesh
    立方体060_1: THREE.Mesh
    立方体060_2: THREE.Mesh
    円柱028: THREE.Mesh
    平面001_1: THREE.Mesh
    平面001_2: THREE.Mesh
    立方体064: THREE.Mesh
    立方体064_1: THREE.Mesh
    ICO球: THREE.Mesh
    円柱030: THREE.Mesh
    円柱031: THREE.Mesh
    円柱049: THREE.Mesh
    円柱049_1: THREE.Mesh
    立方体067: THREE.Mesh
    立方体067_1: THREE.Mesh
    立方体067_2: THREE.Mesh
    立方体067_3: THREE.Mesh
    丸型エレベータードアL003: THREE.Mesh
    丸型エレベータードアR003: THREE.Mesh
    円柱029: THREE.Mesh
    本007: THREE.Mesh
    立方体084: THREE.Mesh
    立方体084_1: THREE.Mesh
    本棚002: THREE.Mesh
    立方体086: THREE.Mesh
    立方体086_1: THREE.Mesh
    本010: THREE.Mesh
    本011: THREE.Mesh
    本012: THREE.Mesh
    立方体068: THREE.Mesh
    立方体068_1: THREE.Mesh
    立方体068_2: THREE.Mesh
    立方体068_3: THREE.Mesh
    立方体048: THREE.Mesh
    立方体075: THREE.Mesh
    立方体075_1: THREE.Mesh
    立方体046: THREE.Mesh
    Floor001: THREE.Mesh
    立方体092: THREE.Mesh
    立方体092_1: THREE.Mesh
    丸型エレベータードアL004: THREE.Mesh
    丸型エレベータードアR004: THREE.Mesh
    円柱033: THREE.Mesh
    円柱034: THREE.Mesh
    円柱035: THREE.Mesh
    円柱036: THREE.Mesh
    立方体095: THREE.Mesh
    立方体095_1: THREE.Mesh
    立方体096: THREE.Mesh
    立方体096_1: THREE.Mesh
    立方体098: THREE.Mesh
    立方体098_1: THREE.Mesh
    立方体099: THREE.Mesh
    立方体099_1: THREE.Mesh
    立方体102: THREE.Mesh
    立方体102_1: THREE.Mesh
    立方体055: THREE.Mesh
    立方体104: THREE.Mesh
    立方体104_1: THREE.Mesh
    立方体105: THREE.Mesh
    立方体105_1: THREE.Mesh
    NURBSパス: THREE.Mesh
    NURBSパス001: THREE.Mesh
    NURBSパス002: THREE.Mesh
    NURBSパス003: THREE.Mesh
    ズボン: THREE.Mesh
    服: THREE.Mesh
    球007: THREE.Mesh
    球007_1: THREE.Mesh
    球005: THREE.Mesh
    ボーン001: THREE.Bone
    ボーン002: THREE.Bone
    ボーン003: THREE.Bone
    ボーン001_1: THREE.Bone
    ボーン002_1: THREE.Bone
    ボーン003_1: THREE.Bone
    ボーン001_2: THREE.Bone
    ボーン002_2: THREE.Bone
    ボーン003_2: THREE.Bone
    ボーン001_3: THREE.Bone
    ボーン002_3: THREE.Bone
    ボーン003_3: THREE.Bone
    ボーン001_4: THREE.Bone
    ボーン002_4: THREE.Bone
    ボーン003_4: THREE.Bone
    ボーン001_5: THREE.Bone
    ボーン002_5: THREE.Bone
    ボーン003_5: THREE.Bone
    ボーン001_6: THREE.Bone
    ボーン002_6: THREE.Bone
    ボーン003_6: THREE.Bone
    ボーン001_7: THREE.Bone
    ボーン002_7: THREE.Bone
    ボーン003_7: THREE.Bone
    ボーン001_8: THREE.Bone
    ボーン002_8: THREE.Bone
    ボーン003_8: THREE.Bone
    ボーン001_9: THREE.Bone
    ボーン002_9: THREE.Bone
    ボーン003_9: THREE.Bone
    ボーン001_10: THREE.Bone
    ボーン002_10: THREE.Bone
    ボーン003_10: THREE.Bone
    ボーン001_11: THREE.Bone
    ボーン002_11: THREE.Bone
    ボーン003_11: THREE.Bone
    ボーン: THREE.Bone
  }
  materials: {
    ['マテリアル.005']: THREE.MeshStandardMaterial
    fire: THREE.MeshStandardMaterial
    Elevator: THREE.MeshStandardMaterial
    Wall: THREE.MeshStandardMaterial
    ['Light.001']: THREE.MeshStandardMaterial
    ['マテリアル.003']: THREE.MeshStandardMaterial
    ['Material.001']: THREE.MeshStandardMaterial
    ['Material.003']: THREE.MeshStandardMaterial
    ['Material.004']: THREE.MeshStandardMaterial
    ['Material.002']: THREE.MeshStandardMaterial
    Wood: THREE.MeshStandardMaterial
    Black: THREE.MeshStandardMaterial
    ['Black.001']: THREE.MeshStandardMaterial
    ['Red.001']: THREE.MeshStandardMaterial
    Orange: THREE.MeshStandardMaterial
    Yellow: THREE.MeshStandardMaterial
    ['Green.001']: THREE.MeshStandardMaterial
    ['Blue.001']: THREE.MeshStandardMaterial
    Purple: THREE.MeshStandardMaterial
    ['Black.002']: THREE.MeshStandardMaterial
    Flame: THREE.MeshStandardMaterial
    Silver: THREE.MeshStandardMaterial
    Earth: THREE.MeshBasicMaterial
    Leaf: THREE.MeshStandardMaterial
    ['マテリアル.004']: THREE.MeshStandardMaterial
    Green: THREE.MeshStandardMaterial
    ['Blue.002']: THREE.MeshStandardMaterial
    Red: THREE.MeshStandardMaterial
    Floor: THREE.MeshStandardMaterial
    Chair: THREE.MeshStandardMaterial
    マテリアル: THREE.MeshStandardMaterial
    Gray: THREE.MeshStandardMaterial
    NewGlass: THREE.MeshStandardMaterial
    Wall_Skills: THREE.MeshStandardMaterial
    Rock: THREE.MeshStandardMaterial
    Blue: THREE.MeshStandardMaterial
    ['SVGMat.010']: THREE.MeshStandardMaterial
    ['python-logo-only']: THREE.MeshStandardMaterial
    OldPaper: THREE.MeshStandardMaterial
    BrawnWood: THREE.MeshStandardMaterial
    ['マテリアル.008']: THREE.MeshStandardMaterial
    ['SVGMat.008']: THREE.MeshStandardMaterial
    ['SVGMat.007']: THREE.MeshStandardMaterial
    ['icons8-typescript-96']: THREE.MeshStandardMaterial
    Sheath: THREE.MeshStandardMaterial
    PureWhite: THREE.MeshStandardMaterial
    Gold: THREE.MeshStandardMaterial
    Brown: THREE.MeshStandardMaterial
    GrayWood: THREE.MeshStandardMaterial
    Rock2: THREE.MeshStandardMaterial
    Desk: THREE.MeshStandardMaterial
    Wall_Projects: THREE.MeshStandardMaterial
    Floor_Projects: THREE.MeshStandardMaterial
    ['Red.002']: THREE.MeshStandardMaterial
    White: THREE.MeshStandardMaterial
    Book_Gray: THREE.MeshStandardMaterial
    Book_Blue: THREE.MeshStandardMaterial
    Book_Red: THREE.MeshStandardMaterial
    Biilding_holo: THREE.MeshStandardMaterial
    Light: THREE.MeshStandardMaterial
    Floor_Gallery: THREE.MeshStandardMaterial
    Wall_Gallery: THREE.MeshStandardMaterial
    Wall_Gallery_Under: THREE.MeshStandardMaterial
    Frame: THREE.MeshStandardMaterial
    Image: THREE.MeshStandardMaterial
    Image_02: THREE.MeshStandardMaterial
    Glass_Gallery: THREE.MeshStandardMaterial
    Image_03: THREE.MeshStandardMaterial
    Material: THREE.MeshBasicMaterial
    ['Black.003']: THREE.MeshStandardMaterial
    Cloth: THREE.MeshStandardMaterial
    ['マテリアル.006']: THREE.MeshStandardMaterial
  }
  animations: GLTFAction[]
}

export default function Model({ activeEvent, ...props }: BuildingProps & JSX.IntrinsicElements['group']) {
  const group = React.useRef<THREE.Group>(null)
  const { scene, animations } = useGLTF('/models/building.glb')
  const clone = React.useMemo(() => SkeletonUtils.clone(scene), [scene])
  const { nodes, materials } = useGraph(clone) as GLTFResult
  const { actions } = useAnimations(animations, group)

  const prevEventRef = useRef<string | null>(null)
  
  useEffect(() => {
    if (!actions || Object.keys(actions).length === 0) return;

    if(activeEvent === 'SECRET') return;

    const currentEvent = activeEvent;
    const prevEvent = prevEventRef.current;

    let mixer: THREE.AnimationMixer | null = null;
    let onFinished: ((e: any) => void | undefined) | null = null;

    if(prevEvent && prevEvent !== currentEvent) {
      const prevStart = actions[`${prevEvent}_Start`];
      const prevLoop = actions[`${prevEvent}_Loop`];

      if (prevLoop?.isRunning()) {
        console.log('Stopping previous loop:', prevLoop.getClip().name);
        prevLoop.stop();
      }

      if (prevStart) {
        prevStart.reset();
        prevStart.time = prevStart.getClip().duration
        prevStart.timeScale = -2
        prevStart.setLoop(THREE.LoopOnce, 1)
        prevStart.clampWhenFinished = true
        prevStart.play()
      }
    }

    if (currentEvent) {
      const start = actions[`${currentEvent}_Start`]
      const loop = actions[`${currentEvent}_Loop`]

      if (start) {
        start.reset()
        start.timeScale = 2
        start.setLoop(THREE.LoopOnce, 1)
        start.clampWhenFinished = true
        start.play()

        mixer = start.getMixer()
        onFinished = (e: any) => {
          if (e.action === start && loop) {
            loop.reset()
            loop.timeScale = 2
            loop.setLoop(THREE.LoopRepeat, Infinity)
            loop.play()
          }
        }
        mixer.addEventListener("finished", onFinished)
      } else if (loop) {
        loop.reset()
        loop.timeScale = 2
        loop.setLoop(THREE.LoopRepeat, Infinity)
        loop.play()
      }
    }
    
    prevEventRef.current = currentEvent;

    return () => {
      if (mixer && onFinished) {
        mixer.removeEventListener("finished", onFinished)
      }
    }
  }, [actions, activeEvent]);

  useEffect(() => {
    if (!actions || Object.keys(actions).length === 0) return;

    const waveReact = actions['wave_react'];
    const wavePython = actions['wave_python'];
    const waveBlender = actions['Wave_blender'];
    const waveTs = actions['wave_ts'];
    const fire = actions['Fire'];
  
    if (waveReact && wavePython && waveBlender && waveTs && fire) {
      waveReact.setLoop(THREE.LoopRepeat, Infinity);
      wavePython.setLoop(THREE.LoopRepeat, Infinity);
      waveBlender.setLoop(THREE.LoopRepeat, Infinity);
      waveTs.setLoop(THREE.LoopRepeat, Infinity);
      fire.setLoop(THREE.LoopRepeat, Infinity);
      waveReact.play();
      wavePython.play();
      waveBlender.play();
      waveTs.play();
      fire.play();
    }
    return () => {
      if (waveReact) waveReact.stop();
      if (wavePython) wavePython.stop();
      if (waveBlender) waveBlender.stop();
      if (waveTs) waveTs.stop();
    };
  }, [actions]);

  return (
    <group ref={group} {...props} dispose={null}>
      <group name="Scene">
        <group name="Event" position={[0.583, -7.451, -0.918]} scale={0.551}>
          <primitive object={nodes.ボーン001} />
          <primitive object={nodes.ボーン002} />
          <primitive object={nodes.ボーン003} />
          <skinnedMesh name="外枠003" geometry={nodes.外枠003.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠003.skeleton} />
        </group>
        <group name="Event001" position={[2.316, -13.502, -0.915]} scale={0.551}>
          <primitive object={nodes.ボーン001_1} />
          <primitive object={nodes.ボーン002_1} />
          <primitive object={nodes.ボーン003_1} />
          <skinnedMesh name="外枠001" geometry={nodes.外枠001.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠001.skeleton} />
        </group>
        <group name="Event002" position={[-0.952, -13.502, 2.357]} scale={0.551}>
          <primitive object={nodes.ボーン001_2} />
          <primitive object={nodes.ボーン002_2} />
          <primitive object={nodes.ボーン003_2} />
          <skinnedMesh name="外枠004" geometry={nodes.外枠004.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠004.skeleton} />
        </group>
        <group name="Event003" position={[-0.952, -13.502, 1.093]} scale={0.551}>
          <primitive object={nodes.ボーン001_3} />
          <primitive object={nodes.ボーン002_3} />
          <primitive object={nodes.ボーン003_3} />
          <skinnedMesh name="外枠006" geometry={nodes.外枠006.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠006.skeleton} />
        </group>
        <group name="Event004" position={[-0.952, -13.502, -0.069]} scale={0.551}>
          <primitive object={nodes.ボーン001_4} />
          <primitive object={nodes.ボーン002_4} />
          <primitive object={nodes.ボーン003_4} />
          <skinnedMesh name="外枠002" geometry={nodes.外枠002.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠002.skeleton} />
        </group>
        <group name="Event005" position={[1.836, -19.484, -0.915]} scale={0.551}>
          <primitive object={nodes.ボーン001_5} />
          <primitive object={nodes.ボーン002_5} />
          <primitive object={nodes.ボーン003_5} />
          <skinnedMesh name="外枠005" geometry={nodes.外枠005.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠005.skeleton} />
        </group>
        <group name="Event006" position={[0.109, -19.484, -0.915]} scale={0.551}>
          <primitive object={nodes.ボーン001_6} />
          <primitive object={nodes.ボーン002_6} />
          <primitive object={nodes.ボーン003_6} />
          <skinnedMesh name="外枠007" geometry={nodes.外枠007.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠007.skeleton} />
        </group>
        <group name="Event007" position={[-0.952, -19.484, 2.053]} scale={0.551}>
          <primitive object={nodes.ボーン001_7} />
          <primitive object={nodes.ボーン002_7} />
          <primitive object={nodes.ボーン003_7} />
          <skinnedMesh name="外枠008" geometry={nodes.外枠008.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠008.skeleton} />
        </group>
        <group name="Event008" position={[2.007, -25.484, -0.915]} scale={0.551}>
          <primitive object={nodes.ボーン001_8} />
          <primitive object={nodes.ボーン002_8} />
          <primitive object={nodes.ボーン003_8} />
          <skinnedMesh name="外枠009" geometry={nodes.外枠009.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠009.skeleton} />
        </group>
        <group name="Event009" position={[0.111, -25.484, -0.915]} scale={0.551}>
          <primitive object={nodes.ボーン001_9} />
          <primitive object={nodes.ボーン002_9} />
          <primitive object={nodes.ボーン003_9} />
          <skinnedMesh name="外枠010" geometry={nodes.外枠010.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠010.skeleton} />
        </group>
        <group name="Event010" position={[-0.952, -25.484, 0.48]} rotation={[0, 1.571, 0]} scale={0.551}>
          <primitive object={nodes.ボーン001_10} />
          <primitive object={nodes.ボーン002_10} />
          <primitive object={nodes.ボーン003_10} />
          <skinnedMesh name="外枠011" geometry={nodes.外枠011.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠011.skeleton} />
        </group>
        <group name="Event011" position={[-0.952, -25.484, 2.255]} rotation={[0, 1.571, 0]} scale={0.551}>
          <primitive object={nodes.ボーン001_11} />
          <primitive object={nodes.ボーン002_11} />
          <primitive object={nodes.ボーン003_11} />
          <skinnedMesh name="外枠012" geometry={nodes.外枠012.geometry} material={materials['マテリアル.005']} skeleton={nodes.外枠012.skeleton} />
        </group>
        <group name="当たり判定001" position={[-1.972, -12.493, 0.395]} scale={[0.5, 1, 2.548]} />
        <group name="当たり判定" position={[0.421, -12.493, -1.975]} rotation={[0, Math.PI / 2, 0]} scale={[0.5, 1, 2.548]} />
        <group name="アーマチュア" position={[0.423, -12.975, -1.916]} scale={0.616}>
          <primitive object={nodes.ボーン} />
          <skinnedMesh name="fire" geometry={nodes.fire.geometry} material={materials.fire} skeleton={nodes.fire.skeleton} />
        </group>
        <mesh name="柵" castShadow receiveShadow geometry={nodes.柵.geometry} material={materials.Elevator} position={[-2.9, 0.46, 2.9]} scale={0.2} />
        <mesh name="屋上床" castShadow receiveShadow geometry={nodes.屋上床.geometry} material={materials.Wall} />
        <mesh name="屋上ライト" castShadow receiveShadow geometry={nodes.屋上ライト.geometry} material={materials['Light.001']} position={[-2.6, 0.2, -2.6]} scale={0.2} />
        <mesh name="屋上ライト001" castShadow receiveShadow geometry={nodes.屋上ライト001.geometry} material={materials['Light.001']} position={[2.6, 0.2, 2.6]} rotation={[Math.PI, 0, Math.PI]} scale={0.2} />
        <mesh name="屋上ライト002" castShadow receiveShadow geometry={nodes.屋上ライト002.geometry} material={materials['Light.001']} position={[-2.6, 0.2, 2.6]} rotation={[0, Math.PI / 2, 0]} scale={0.2} />
        <mesh name="屋上ライト003" castShadow receiveShadow geometry={nodes.屋上ライト003.geometry} material={materials['Light.001']} position={[2.6, 0.2, -2.6]} rotation={[0, -1.571, 0]} scale={0.2} />
        <mesh name="望遠鏡" castShadow receiveShadow geometry={nodes.望遠鏡.geometry} material={materials['マテリアル.003']} position={[1.61, 1.792, -1.529]} rotation={[-1.189, -0.234, -0.523]} />
        <mesh name="丸型エレベータードアL001" castShadow receiveShadow geometry={nodes.丸型エレベータードアL001.geometry} material={materials.Elevator} position={[-1.689, 4.009, -1.7]} rotation={[0, Math.PI / 4, 0]} />
        <mesh name="丸型エレベータードアR001" castShadow receiveShadow geometry={nodes.丸型エレベータードアR001.geometry} material={materials.Elevator} position={[-1.689, 4.009, -1.7]} rotation={[0, Math.PI / 4, 0]} />
        <mesh name="エレベーター" castShadow receiveShadow geometry={nodes.エレベーター.geometry} material={materials.Elevator} position={[-1.689, 4.009, -1.7]} rotation={[0, Math.PI / 4, 0]} />
        <group name="円柱016" position={[-0.598, 0.469, -1.878]} rotation={[0, 0.265, 0]}>
          <mesh name="円柱030_1" castShadow receiveShadow geometry={nodes.円柱030_1.geometry} material={materials.Elevator} />
          <mesh name="円柱030_2" castShadow receiveShadow geometry={nodes.円柱030_2.geometry} material={materials.Wall} />
        </group>
        <mesh name="Cube004" castShadow receiveShadow geometry={nodes.Cube004.geometry} material={materials['Material.001']} position={[0.523, -6.768, -2.069]} scale={[0.158, 0.173, 0.186]} />
        <group name="Cube005" position={[0.523, -6.589, -2.323]} rotation={[1.137, 0, 0]} scale={[0.158, 0.173, 0.186]}>
          <mesh name="Cube005_1" castShadow receiveShadow geometry={nodes.Cube005_1.geometry} material={materials['Material.001']} />
          <mesh name="Cube005_2" castShadow receiveShadow geometry={nodes.Cube005_2.geometry} material={materials['Material.003']} />
          <mesh name="Cube005_3" castShadow receiveShadow geometry={nodes.Cube005_3.geometry} material={materials['Material.004']} />
        </group>
        <mesh name="Cube006" castShadow receiveShadow geometry={nodes.Cube006.geometry} material={materials['Material.002']} position={[0.326, -6.76, -2.182]} scale={[0.011, 0.003, 0.011]} />
        <mesh name="Cube007" castShadow receiveShadow geometry={nodes.Cube007.geometry} material={materials['Material.001']} position={[0.523, -6.761, -2.012]} scale={[0.158, 0.173, 0.186]} />
        <mesh name="立方体012" castShadow receiveShadow geometry={nodes.立方体012.geometry} material={materials.Wood} position={[0.548, -6.845, -2.101]} scale={0.334} />
        <group name="円柱013" position={[0.007, -6.735, -2.281]} rotation={[-Math.PI, 0.574, -Math.PI]} scale={0.586}>
          <mesh name="円柱022" castShadow receiveShadow geometry={nodes.円柱022.geometry} material={materials.Wood} />
          <mesh name="円柱022_1" castShadow receiveShadow geometry={nodes.円柱022_1.geometry} material={materials.Black} />
          <mesh name="円柱022_2" castShadow receiveShadow geometry={nodes.円柱022_2.geometry} material={materials['Black.001']} />
          <mesh name="円柱022_3" castShadow receiveShadow geometry={nodes.円柱022_3.geometry} material={materials['Light.001']} />
        </group>
        <group name="ハノイの塔" position={[-2.062, -6.56, 1.368]} rotation={[0, Math.PI / 2, 0]} scale={0.415}>
          <mesh name="立方体020" castShadow receiveShadow geometry={nodes.立方体020.geometry} material={materials.Wood} />
          <mesh name="立方体020_1" castShadow receiveShadow geometry={nodes.立方体020_1.geometry} material={materials['Red.001']} />
          <mesh name="立方体020_2" castShadow receiveShadow geometry={nodes.立方体020_2.geometry} material={materials.Orange} />
          <mesh name="立方体020_3" castShadow receiveShadow geometry={nodes.立方体020_3.geometry} material={materials.Yellow} />
          <mesh name="立方体020_4" castShadow receiveShadow geometry={nodes.立方体020_4.geometry} material={materials['Green.001']} />
          <mesh name="立方体020_5" castShadow receiveShadow geometry={nodes.立方体020_5.geometry} material={materials['Blue.001']} />
          <mesh name="立方体020_6" castShadow receiveShadow geometry={nodes.立方体020_6.geometry} material={materials.Purple} />
        </group>
        <mesh name="マウス002" castShadow receiveShadow geometry={nodes.マウス002.geometry} material={materials['Black.002']} position={[0.961, -6.751, -2.015]} scale={0.087} />
        <mesh name="円柱014" castShadow receiveShadow geometry={nodes.円柱014.geometry} material={materials.Flame} position={[-2.003, -7.311, -0.06]} rotation={[0, -0.656, 0]} scale={1.168} />
        <group name="地球儀" position={[-1.992, -6.271, 2.302]} rotation={[1.235, -0.15, -1.165]} scale={0.415}>
          <mesh name="円柱023" castShadow receiveShadow geometry={nodes.円柱023.geometry} material={materials.Wood} />
          <mesh name="円柱023_1" castShadow receiveShadow geometry={nodes.円柱023_1.geometry} material={materials.Silver} />
          <mesh name="円柱023_2" castShadow receiveShadow geometry={nodes.円柱023_2.geometry} material={materials.Earth} />
        </group>
        <mesh name="立方体013" castShadow receiveShadow geometry={nodes.立方体013.geometry} material={materials.Leaf} position={[-1.967, -7.281, -0.105]} rotation={[-0.219, -0.644, -0.133]} scale={[0.102, 0.135, 0.135]} />
        <mesh name="立方体014" castShadow receiveShadow geometry={nodes.立方体014.geometry} material={materials.Leaf} position={[-2.06, -7.281, -0.067]} rotation={[-0.929, 1.352, 0.918]} scale={[0.102, 0.135, 0.135]} />
        <mesh name="立方体015" castShadow receiveShadow geometry={nodes.立方体015.geometry} material={materials.Leaf} position={[-1.981, -7.281, -0.006]} rotation={[-2.953, -0.385, -3.07]} scale={[0.102, 0.135, 0.135]} />
        <mesh name="立方体021" castShadow receiveShadow geometry={nodes.立方体021.geometry} material={materials.Leaf} position={[-2.008, -7.149, -0.101]} rotation={[-0.234, 0.727, 0.157]} scale={[0.086, 0.163, 0.116]} />
        <mesh name="立方体022" castShadow receiveShadow geometry={nodes.立方体022.geometry} material={materials.Leaf} position={[-2.019, -7.149, -0.017]} rotation={[-2.923, 0.644, 3.009]} scale={[0.073, 0.137, 0.097]} />
        <mesh name="立方体023" castShadow receiveShadow geometry={nodes.立方体023.geometry} material={materials.Leaf} position={[-2.008, -7.149, -0.101]} rotation={[-0.686, -1.293, -0.667]} scale={[0.073, 0.138, 0.098]} />
        <group name="立方体024" position={[-2.042, -7.025, 1.732]} rotation={[0, Math.PI / 2, 0]} scale={0.415}>
          <mesh name="立方体040_1" castShadow receiveShadow geometry={nodes.立方体040_1.geometry} material={materials.Wood} />
          <mesh name="立方体040_2" castShadow receiveShadow geometry={nodes.立方体040_2.geometry} material={materials['マテリアル.004']} />
        </group>
        <mesh name="本001" castShadow receiveShadow geometry={nodes.本001.geometry} material={materials.Green} position={[1.541, -5.6, -2.038]} />
        <mesh name="本棚001" castShadow receiveShadow geometry={nodes.本棚001.geometry} material={materials.Wood} position={[2.14, -6.409, -2.161]} scale={0.179} />
        <group name="本002" position={[1.541, -6.014, -2.038]}>
          <mesh name="立方体044_1" castShadow receiveShadow geometry={nodes.立方体044_1.geometry} material={materials['Blue.002']} />
          <mesh name="立方体044_2" castShadow receiveShadow geometry={nodes.立方体044_2.geometry} material={materials.Red} />
        </group>
        <mesh name="Floor" castShadow receiveShadow geometry={nodes.Floor.geometry} material={materials.Floor} position={[0.259, -7.5, 0.259]} rotation={[0, Math.PI / 2, 0]} />
        <group name="本003" position={[1.541, -6.436, -2.038]}>
          <mesh name="立方体045_1" castShadow receiveShadow geometry={nodes.立方体045_1.geometry} material={materials.Green} />
          <mesh name="立方体045_2" castShadow receiveShadow geometry={nodes.立方体045_2.geometry} material={materials['Blue.002']} />
        </group>
        <mesh name="本004" castShadow receiveShadow geometry={nodes.本004.geometry} material={materials.Green} position={[1.541, -6.855, -2.038]} />
        <mesh name="本005" castShadow receiveShadow geometry={nodes.本005.geometry} material={materials.Red} position={[2.277, -6.855, -2.038]} />
        <mesh name="本006" castShadow receiveShadow geometry={nodes.本006.geometry} material={materials.Green} position={[1.541, -7.27, -2.038]} />
        <group name="建物" position={[0, -5, 0]}>
          <mesh name="立方体022_1" castShadow receiveShadow geometry={nodes.立方体022_1.geometry} material={materials.Wall} />
          <mesh name="立方体022_2" castShadow receiveShadow geometry={nodes.立方体022_2.geometry} material={materials.Flame} />
        </group>
        <group name="円柱015" position={[0.59, -7.488, -1.664]} rotation={[0, -0.328, 0]} scale={1.115}>
          <mesh name="円柱025_1" castShadow receiveShadow geometry={nodes.円柱025_1.geometry} material={materials.Chair} />
          <mesh name="円柱025_2" castShadow receiveShadow geometry={nodes.円柱025_2.geometry} material={materials.マテリアル} />
        </group>
        <mesh name="立方体025" castShadow receiveShadow geometry={nodes.立方体025.geometry} material={materials.Gray} position={[1.934, -6.99, -2.018]} rotation={[Math.PI, 0, Math.PI]} scale={[0.059, 0.007, 0.072]} />
        <mesh name="丸型エレベータードアL" castShadow receiveShadow geometry={nodes.丸型エレベータードアL.geometry} material={materials.Elevator} position={[-1.689, -3.7, -1.7]} rotation={[0, Math.PI / 4, 0]} scale={[0.99, 1, 0.99]} />
        <mesh name="丸型エレベータードアR" castShadow receiveShadow geometry={nodes.丸型エレベータードアR.geometry} material={materials.Elevator} position={[-1.689, -3.7, -1.7]} rotation={[0, Math.PI / 4, 0]} scale={[0.99, 1, 0.99]} />
        <mesh name="円柱012" castShadow receiveShadow geometry={nodes.円柱012.geometry} material={materials.Elevator} position={[-1.689, -3.7, -1.7]} rotation={[0, Math.PI / 4, 0]} />
        <mesh name="ガラス窓" castShadow receiveShadow geometry={nodes.ガラス窓.geometry} material={materials.NewGlass} position={[0, -5, 0]} />
        <mesh name="建物001" castShadow receiveShadow geometry={nodes.建物001.geometry} material={materials.Wall_Skills} position={[0, -11, 0]} />
        <group name="平面" position={[0.258, -13.526, 0.22]}>
          <mesh name="平面_1" castShadow receiveShadow geometry={nodes.平面_1.geometry} material={materials.Wall_Skills} />
          <mesh name="平面_2" castShadow receiveShadow geometry={nodes.平面_2.geometry} material={materials.Rock} />
        </group>
        <mesh name="丸型エレベータードアL002" castShadow receiveShadow geometry={nodes.丸型エレベータードアL002.geometry} material={materials.Elevator} position={[-1.689, -9.706, -1.7]} rotation={[0, Math.PI / 4, 0]} scale={[0.99, 1, 0.99]} />
        <mesh name="丸型エレベータードアR002" castShadow receiveShadow geometry={nodes.丸型エレベータードアR002.geometry} material={materials.Elevator} position={[-1.689, -9.706, -1.7]} rotation={[0, Math.PI / 4, 0]} scale={[0.99, 1, 0.99]} />
        <mesh name="円柱017" castShadow receiveShadow geometry={nodes.円柱017.geometry} material={materials.Elevator} position={[-1.689, -9.706, -1.7]} rotation={[0, Math.PI / 4, 0]} />
        <mesh name="立方体044" castShadow receiveShadow geometry={nodes.立方体044.geometry} material={materials.Wood} position={[-2.429, -10.155, 1.181]} rotation={[0, Math.PI / 2, 0]} scale={[1, 0.07, 0.051]} />
        <mesh name="円柱025" castShadow receiveShadow geometry={nodes.円柱025.geometry} material={materials.Silver} position={[-2.354, -10.162, 0.638]} rotation={[Math.PI / 2, 0, -Math.PI / 2]} scale={[0.02, 0.024, 0.02]} />
        <mesh name="円柱026" castShadow receiveShadow geometry={nodes.円柱026.geometry} material={materials.Silver} position={[-2.354, -10.162, 1.11]} rotation={[Math.PI / 2, 0, -Math.PI / 2]} scale={[0.02, 0.024, 0.02]} />
        <mesh name="円柱027" castShadow receiveShadow geometry={nodes.円柱027.geometry} material={materials.Silver} position={[-2.354, -10.162, 1.616]} rotation={[Math.PI / 2, 0, -Math.PI / 2]} scale={[0.02, 0.024, 0.02]} />
        <mesh name="立方体029" castShadow receiveShadow geometry={nodes.立方体029.geometry} material={materials.Rock} position={[-2.239, -12.911, -2.55]} />
        <mesh name="立方体027" castShadow receiveShadow geometry={nodes.立方体027.geometry} material={materials.Rock} position={[-2.545, -12.911, 2.685]} rotation={[0, Math.PI / 2, 0]} />
        <group name="立方体028" position={[-1.994, -12.285, 1.048]} rotation={[-0.697, 0.86, 0.566]} scale={0.395}>
          <mesh name="立方体036_1" castShadow receiveShadow geometry={nodes.立方体036_1.geometry} material={materials.Rock} />
          <mesh name="立方体036_2" castShadow receiveShadow geometry={nodes.立方体036_2.geometry} material={materials.Blue} />
          <mesh name="立方体036_3" castShadow receiveShadow geometry={nodes.立方体036_3.geometry} material={materials['SVGMat.010']} />
        </group>
        <group name="python-logo-only" position={[-1.809, -12.328, -0.052]} rotation={[1.389, 0.374, -1.105]} scale={0.546}>
          <mesh name="python-logo-only_1" castShadow receiveShadow geometry={nodes['python-logo-only_1'].geometry} material={materials['python-logo-only']} />
          <mesh name="python-logo-only_2" castShadow receiveShadow geometry={nodes['python-logo-only_2'].geometry} material={materials.OldPaper} />
        </group>
        <group name="立方体030" position={[2.279, -11.914, -2.11]} rotation={[-0.361, 0.505, 0.181]} scale={0.521}>
          <mesh name="立方体037_1" castShadow receiveShadow geometry={nodes.立方体037_1.geometry} material={materials.BrawnWood} />
          <mesh name="立方体037_2" castShadow receiveShadow geometry={nodes.立方体037_2.geometry} material={materials['マテリアル.008']} />
          <mesh name="立方体037_3" castShadow receiveShadow geometry={nodes.立方体037_3.geometry} material={materials['SVGMat.008']} />
          <mesh name="立方体037_4" castShadow receiveShadow geometry={nodes.立方体037_4.geometry} material={materials['SVGMat.007']} />
        </group>
        <group name="Sheath" position={[-1.943, -11.848, 2.526]} rotation={[1.911, 0.471, -1.129]} scale={0.253}>
          <mesh name="icons8-typescript-96" castShadow receiveShadow geometry={nodes['icons8-typescript-96'].geometry} material={materials['icons8-typescript-96']} />
          <mesh name="icons8-typescript-96_1" castShadow receiveShadow geometry={nodes['icons8-typescript-96_1'].geometry} material={materials.Sheath} />
          <mesh name="icons8-typescript-96_2" castShadow receiveShadow geometry={nodes['icons8-typescript-96_2'].geometry} material={materials.PureWhite} />
          <mesh name="icons8-typescript-96_3" castShadow receiveShadow geometry={nodes['icons8-typescript-96_3'].geometry} material={materials.Gold} />
          <mesh name="icons8-typescript-96_4" castShadow receiveShadow geometry={nodes['icons8-typescript-96_4'].geometry} material={materials.Brown} />
          <mesh name="icons8-typescript-96_5" castShadow receiveShadow geometry={nodes['icons8-typescript-96_5'].geometry} material={materials.Rock} />
        </group>
        <group name="立方体058" position={[-2.196, -19.159, -0.074]} rotation={[0, Math.PI / 2, 0]} scale={[1, 1.056, 1]}>
          <mesh name="立方体100" castShadow receiveShadow geometry={nodes.立方体100.geometry} material={materials.GrayWood} />
          <mesh name="立方体100_1" castShadow receiveShadow geometry={nodes.立方体100_1.geometry} material={materials.Silver} />
        </group>
        <mesh name="立方体035" castShadow receiveShadow geometry={nodes.立方体035.geometry} material={materials.Rock} position={[0.423, -12.326, -1.897]} scale={[1.186, 1.186, 1.539]} />
        <mesh name="立方体036" castShadow receiveShadow geometry={nodes.立方体036.geometry} material={materials.Rock} position={[0.423, -13.104, -1.933]} scale={[2.869, 1.186, 1.041]} />
        <mesh name="立方体037" castShadow receiveShadow geometry={nodes.立方体037.geometry} material={materials.Rock} position={[-0.142, -13.349, -1.933]} scale={[0.574, 1.186, 1.041]} />
        <mesh name="立方体038" castShadow receiveShadow geometry={nodes.立方体038.geometry} material={materials.Rock} position={[0.987, -13.349, -1.933]} scale={[0.574, 1.186, 1.041]} />
        <mesh name="立方体039" castShadow receiveShadow geometry={nodes.立方体039.geometry} material={materials.Rock} position={[0.273, -12.938, -2.113]} rotation={[0, -0.475, 0]} scale={[0.265, 0.414, 0.287]} />
        <mesh name="立方体040" castShadow receiveShadow geometry={nodes.立方体040.geometry} material={materials.Rock} position={[0.472, -12.938, -1.811]} rotation={[0, -0.475, 0]} scale={[0.265, 0.414, 0.287]} />
        <mesh name="立方体041" castShadow receiveShadow geometry={nodes.立方体041.geometry} material={materials.Rock} position={[0.241, -12.936, -1.857]} rotation={[Math.PI, -1.398, Math.PI]} scale={[0.265, 0.414, 0.287]} />
        <mesh name="立方体042" castShadow receiveShadow geometry={nodes.立方体042.geometry} material={materials.Rock} position={[0.566, -12.931, -2.003]} rotation={[Math.PI, -0.384, Math.PI]} scale={[0.326, 0.508, 0.611]} />
        <mesh name="立方体043" castShadow receiveShadow geometry={nodes.立方体043.geometry} material={materials.Rock} position={[0.306, -12.938, -1.782]} rotation={[Math.PI, -1.349, Math.PI]} scale={[0.265, 0.414, 0.287]} />
        <group name="円柱020" position={[-1.97, -12.929, 2.349]}>
          <mesh name="円柱037" castShadow receiveShadow geometry={nodes.円柱037.geometry} material={materials.Rock} />
          <mesh name="円柱037_1" castShadow receiveShadow geometry={nodes.円柱037_1.geometry} material={materials.Rock2} />
        </group>
        <group name="円柱018" position={[-1.97, -12.929, 1.072]}>
          <mesh name="円柱042" castShadow receiveShadow geometry={nodes.円柱042.geometry} material={materials.Rock} />
          <mesh name="円柱042_1" castShadow receiveShadow geometry={nodes.円柱042_1.geometry} material={materials.Rock2} />
        </group>
        <group name="円柱019" position={[-1.97, -12.929, -0.099]}>
          <mesh name="円柱043" castShadow receiveShadow geometry={nodes.円柱043.geometry} material={materials.Rock} />
          <mesh name="円柱043_1" castShadow receiveShadow geometry={nodes.円柱043_1.geometry} material={materials.Rock2} />
        </group>
        <group name="円柱024" position={[2.331, -12.929, -1.971]}>
          <mesh name="円柱044" castShadow receiveShadow geometry={nodes.円柱044.geometry} material={materials.Rock} />
          <mesh name="円柱044_1" castShadow receiveShadow geometry={nodes.円柱044_1.geometry} material={materials.Rock2} />
        </group>
        <group name="建物002" position={[0, -17, 0]}>
          <mesh name="立方体060" castShadow receiveShadow geometry={nodes.立方体060.geometry} material={materials.Wall} />
          <mesh name="立方体060_1" castShadow receiveShadow geometry={nodes.立方体060_1.geometry} material={materials.Desk} />
          <mesh name="立方体060_2" castShadow receiveShadow geometry={nodes.立方体060_2.geometry} material={materials.Wall_Projects} />
        </group>
        <mesh name="円柱028" castShadow receiveShadow geometry={nodes.円柱028.geometry} material={materials.Elevator} position={[-1.689, -15.706, -1.7]} rotation={[0, Math.PI / 4, 0]} />
        <group name="平面001" position={[0.258, -19.526, 0.22]}>
          <mesh name="平面001_1" castShadow receiveShadow geometry={nodes.平面001_1.geometry} material={materials.Wall_Projects} />
          <mesh name="平面001_2" castShadow receiveShadow geometry={nodes.平面001_2.geometry} material={materials.Floor_Projects} />
        </group>
        <group name="立方体045" position={[1.836, -19.446, -1.973]} scale={0.941}>
          <mesh name="立方体064" castShadow receiveShadow geometry={nodes.立方体064.geometry} material={materials.Gray} />
          <mesh name="立方体064_1" castShadow receiveShadow geometry={nodes.立方体064_1.geometry} material={materials['Light.001']} />
        </group>
        <mesh name="ICO球" castShadow receiveShadow geometry={nodes.ICO球.geometry} material={materials['Red.002']} position={[-0.087, -18.488, -1.665]} scale={0.032} />
        <mesh name="円柱030" castShadow receiveShadow geometry={nodes.円柱030.geometry} material={materials['Red.002']} position={[0.192, -18.643, -1.665]} />
        <mesh name="円柱031" castShadow receiveShadow geometry={nodes.円柱031.geometry} material={materials.Black} position={[-0.087, -18.558, -1.665]} scale={[1, 0.722, 1]} />
        <group name="円柱032" position={[-0.087, -18.618, -1.665]}>
          <mesh name="円柱049" castShadow receiveShadow geometry={nodes.円柱049.geometry} material={materials.Black} />
          <mesh name="円柱049_1" castShadow receiveShadow geometry={nodes.円柱049_1.geometry} material={materials.White} />
        </group>
        <group name="Game" position={[0.109, -19.456, -1.974]} scale={0.782}>
          <mesh name="立方体067" castShadow receiveShadow geometry={nodes.立方体067.geometry} material={materials.Blue} />
          <mesh name="立方体067_1" castShadow receiveShadow geometry={nodes.立方体067_1.geometry} material={materials.Gray} />
          <mesh name="立方体067_2" castShadow receiveShadow geometry={nodes.立方体067_2.geometry} material={materials['Light.001']} />
          <mesh name="立方体067_3" castShadow receiveShadow geometry={nodes.立方体067_3.geometry} material={materials.Black} />
        </group>
        <mesh name="丸型エレベータードアL003" castShadow receiveShadow geometry={nodes.丸型エレベータードアL003.geometry} material={materials.Elevator} position={[-1.689, -15.706, -1.7]} rotation={[0, Math.PI / 4, 0]} scale={[0.99, 1, 0.99]} />
        <mesh name="丸型エレベータードアR003" castShadow receiveShadow geometry={nodes.丸型エレベータードアR003.geometry} material={materials.Elevator} position={[-1.689, -15.706, -1.7]} rotation={[0, Math.PI / 4, 0]} scale={[0.99, 1, 0.99]} />
        <mesh name="円柱029" castShadow receiveShadow geometry={nodes.円柱029.geometry} material={materials['Red.002']} position={[0.33, -18.643, -1.665]} />
        <mesh name="本007" castShadow receiveShadow geometry={nodes.本007.geometry} material={materials.Book_Gray} position={[-2.01, -16.9, 1.406]} rotation={[0, Math.PI / 2, 0]} />
        <group name="本008" position={[-2.01, -17.736, 0.53]} rotation={[0, Math.PI / 2, 0]}>
          <mesh name="立方体084" castShadow receiveShadow geometry={nodes.立方体084.geometry} material={materials.Book_Gray} />
          <mesh name="立方体084_1" castShadow receiveShadow geometry={nodes.立方体084_1.geometry} material={materials.Book_Blue} />
        </group>
        <mesh name="本棚002" castShadow receiveShadow geometry={nodes.本棚002.geometry} material={materials.GrayWood} position={[-2.208, -17.709, -0.069]} rotation={[0, Math.PI / 2, 0]} scale={0.179} />
        <group name="本009" position={[-2.01, -17.322, 1.396]} rotation={[0, Math.PI / 2, 0]}>
          <mesh name="立方体086" castShadow receiveShadow geometry={nodes.立方体086.geometry} material={materials.Book_Gray} />
          <mesh name="立方体086_1" castShadow receiveShadow geometry={nodes.立方体086_1.geometry} material={materials.Book_Blue} />
        </group>
        <mesh name="本010" castShadow receiveShadow geometry={nodes.本010.geometry} material={materials.Book_Gray} position={[-2.01, -17.325, 0.086]} rotation={[0, Math.PI / 2, 0]} />
        <mesh name="本011" castShadow receiveShadow geometry={nodes.本011.geometry} material={materials.Book_Red} position={[-2.094, -18.156, -0.159]} rotation={[0, Math.PI / 2, 0]} />
        <mesh name="本012" castShadow receiveShadow geometry={nodes.本012.geometry} material={materials.Book_Gray} position={[-2.01, -18.154, 0.52]} rotation={[0, Math.PI / 2, 0]} />
        <group name="立方体047" position={[-1.973, -18.559, 2.053]} rotation={[0, 0.284, 0]}>
          <mesh name="立方体068" castShadow receiveShadow geometry={nodes.立方体068.geometry} material={materials.Biilding_holo} />
          <mesh name="立方体068_1" castShadow receiveShadow geometry={nodes.立方体068_1.geometry} material={materials.Light} />
          <mesh name="立方体068_2" castShadow receiveShadow geometry={nodes.立方体068_2.geometry} material={materials.Elevator} />
          <mesh name="立方体068_3" castShadow receiveShadow geometry={nodes.立方体068_3.geometry} material={materials['マテリアル.003']} />
        </group>
        <mesh name="立方体048" castShadow receiveShadow geometry={nodes.立方体048.geometry} material={nodes.立方体048.material} position={[-1.973, -19.444, 2.053]} />
        <group name="立方体049" position={[1.836, -17.723, -1.973]}>
          <mesh name="立方体075" castShadow receiveShadow geometry={nodes.立方体075.geometry} material={materials.Silver} />
          <mesh name="立方体075_1" castShadow receiveShadow geometry={nodes.立方体075_1.geometry} material={materials['Light.001']} />
        </group>
        <mesh name="立方体046" castShadow receiveShadow geometry={nodes.立方体046.geometry} material={materials['Black.002']} position={[-2.454, -18.528, 1.143]} scale={[0.056, 0.972, 0.464]} />
        <mesh name="Floor001" castShadow receiveShadow geometry={nodes.Floor001.geometry} material={materials.Floor_Gallery} position={[0.259, -25.5, 0.259]} rotation={[0, Math.PI / 2, 0]} />
        <group name="建物003" position={[0, -23, 0]}>
          <mesh name="立方体092" castShadow receiveShadow geometry={nodes.立方体092.geometry} material={materials.Wall_Gallery} />
          <mesh name="立方体092_1" castShadow receiveShadow geometry={nodes.立方体092_1.geometry} material={materials.Wall_Gallery_Under} />
        </group>
        <mesh name="丸型エレベータードアL004" castShadow receiveShadow geometry={nodes.丸型エレベータードアL004.geometry} material={materials.Elevator} position={[-1.689, -21.706, -1.7]} rotation={[0, Math.PI / 4, 0]} scale={[0.99, 1, 0.99]} />
        <mesh name="丸型エレベータードアR004" castShadow receiveShadow geometry={nodes.丸型エレベータードアR004.geometry} material={materials.Elevator} position={[-1.689, -21.706, -1.7]} rotation={[0, Math.PI / 4, 0]} scale={[0.99, 1, 0.99]} />
        <mesh name="円柱033" castShadow receiveShadow geometry={nodes.円柱033.geometry} material={materials.Elevator} position={[-1.689, -21.706, -1.7]} rotation={[0, Math.PI / 4, 0]} />
        <mesh name="円柱034" castShadow receiveShadow geometry={nodes.円柱034.geometry} material={materials.Gray} position={[0.111, -22.727, -2.242]} rotation={[-2.742, -Math.PI / 2, 0]} />
        <mesh name="円柱035" castShadow receiveShadow geometry={nodes.円柱035.geometry} material={materials.Gray} position={[2.007, -22.727, -2.242]} rotation={[-2.742, -Math.PI / 2, 0]} />
        <mesh name="円柱036" castShadow receiveShadow geometry={nodes.円柱036.geometry} material={materials.Gray} position={[-2.241, -22.727, 0.48]} rotation={[0, 0, 2.742]} />
        <group name="立方体050" position={[2.007, -24.022, -2.492]}>
          <mesh name="立方体095" castShadow receiveShadow geometry={nodes.立方体095.geometry} material={materials.Frame} />
          <mesh name="立方体095_1" castShadow receiveShadow geometry={nodes.立方体095_1.geometry} material={materials.Image} />
        </group>
        <group name="立方体051" position={[2.007, -24.877, -2.526]}>
          <mesh name="立方体096" castShadow receiveShadow geometry={nodes.立方体096.geometry} material={materials.White} />
          <mesh name="立方体096_1" castShadow receiveShadow geometry={nodes.立方体096_1.geometry} material={materials.Black} />
        </group>
        <group name="立方体052" position={[0.111, -24.022, -2.492]}>
          <mesh name="立方体098" castShadow receiveShadow geometry={nodes.立方体098.geometry} material={materials.Frame} />
          <mesh name="立方体098_1" castShadow receiveShadow geometry={nodes.立方体098_1.geometry} material={materials.Image_02} />
        </group>
        <group name="立方体053" position={[0.111, -24.877, -2.525]}>
          <mesh name="立方体099" castShadow receiveShadow geometry={nodes.立方体099.geometry} material={materials.White} />
          <mesh name="立方体099_1" castShadow receiveShadow geometry={nodes.立方体099_1.geometry} material={materials.Black} />
        </group>
        <group name="立方体054" position={[-1.989, -25.284, 2.255]} rotation={[0, Math.PI / 2, 0]}>
          <mesh name="立方体102" castShadow receiveShadow geometry={nodes.立方体102.geometry} material={materials['Light.001']} />
          <mesh name="立方体102_1" castShadow receiveShadow geometry={nodes.立方体102_1.geometry} material={materials.White} />
        </group>
        <mesh name="立方体055" castShadow receiveShadow geometry={nodes.立方体055.geometry} material={materials.Glass_Gallery} position={[-1.989, -25.284, 2.255]} rotation={[0, Math.PI / 2, 0]} />
        <group name="立方体056" position={[-2.492, -24.022, 0.48]} rotation={[0, Math.PI / 2, 0]}>
          <mesh name="立方体104" castShadow receiveShadow geometry={nodes.立方体104.geometry} material={materials.Frame} />
          <mesh name="立方体104_1" castShadow receiveShadow geometry={nodes.立方体104_1.geometry} material={materials.Image_03} />
        </group>
        <group name="立方体057" position={[-2.525, -24.877, 0.48]} rotation={[0, Math.PI / 2, 0]}>
          <mesh name="立方体105" castShadow receiveShadow geometry={nodes.立方体105.geometry} material={materials.White} />
          <mesh name="立方体105_1" castShadow receiveShadow geometry={nodes.立方体105_1.geometry} material={materials.Black} />
        </group>
        <mesh name="NURBSパス" castShadow receiveShadow geometry={nodes.NURBSパス.geometry} material={materials.Material} position={[-1.743, -24.092, 2.389]} rotation={[0, 1.043, Math.PI / 2]} scale={0.272} />
        <mesh name="NURBSパス001" castShadow receiveShadow geometry={nodes.NURBSパス001.geometry} material={materials.Material} position={[-1.956, -25.043, 2.265]} rotation={[0, 1.043, -Math.PI / 2]} scale={0.272} />
        <mesh name="NURBSパス002" castShadow receiveShadow geometry={nodes.NURBSパス002.geometry} material={materials.Material} position={[-1.956, -25.043, 2.265]} rotation={[0, 1.043, -Math.PI / 2]} scale={0.272} />
        <mesh name="NURBSパス003" castShadow receiveShadow geometry={nodes.NURBSパス003.geometry} material={materials.Material} position={[-2.016, -25.043, 2.276]} rotation={[0, 1.043, -Math.PI / 2]} scale={0.272} />
        <mesh name="ズボン" castShadow receiveShadow geometry={nodes.ズボン.geometry} material={materials['Black.003']} position={[-1.956, -25.043, 2.265]} rotation={[0, 1.043, 0]} scale={0.272} />
        <mesh name="服" castShadow receiveShadow geometry={nodes.服.geometry} material={materials.Cloth} position={[-1.956, -25.035, 2.265]} rotation={[0, 1.043, 0]} scale={0.272} />
        <group name="球004" position={[-1.956, -25.043, 2.265]} rotation={[0, 1.043, 0]} scale={0.272}>
          <mesh name="球007" castShadow receiveShadow geometry={nodes.球007.geometry} material={materials['Black.003']} />
          <mesh name="球007_1" castShadow receiveShadow geometry={nodes.球007_1.geometry} material={materials['マテリアル.006']} />
        </group>
        <mesh name="球005" castShadow receiveShadow geometry={nodes.球005.geometry} material={materials.Material} position={[-1.956, -24.135, 2.265]} rotation={[0, 1.043, 0]} scale={0.275} />
      </group>
    </group>
  )
}

useGLTF.preload('/models/building.glb')
